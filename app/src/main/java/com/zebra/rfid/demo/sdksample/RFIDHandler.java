...
class RFIDHandler implements Readers.RFIDReaderEventHandler {
    ...
    private EventHandler eventHandler, externHandler;
    ...
    public String Test1(RFIDReader myReader) {
...
//            RADIO_POWER_STATE radio =  myReader.Config.getRadioPowerState();
            ...
            config = myReader.Config.Antennas.getAntennaRfConfig(1);
            config.setTransmitPowerIndex(100);
            config.setrfModeTableIndex(0);
            config.setTari(0);
            myReader.Config.Antennas.setAntennaRfConfig(1, config);
    ...
    public String Test2(RFIDReader myReader) {
        // check reader connection
//        if (!isReaderConnected())
//            return "Not connected";
        // Set the singulation control to S2 which will read each tag once only
        try {
            Antennas.SingulationControl s1_singulationControl = myReader.Config.Antennas.getSingulationControl(1);
            s1_singulationControl.setSession(SESSION.SESSION_S2);
            s1_singulationControl.Action.setInventoryState(INVENTORY_STATE.INVENTORY_STATE_A);
            s1_singulationControl.Action.setSLFlag(SL_FLAG.SL_ALL);
            myReader.Config.Antennas.setSingulationControl(1, s1_singulationControl);
        } catch (InvalidUsageException e) {
            e.printStackTrace();
        } catch (OperationFailureException e) {
            e.printStackTrace();
    ...
    public String Defaults(RFIDReader myReader) {
        // check reader connection
//        if (!isReaderConnected())
//            return "Not connected";;
        try {
    ...
    public String Test3(RFIDReader myReader) {
        // check reader connection
//        if (!isReaderConnected())
//            return "Not connected";
        // Set the singulation control to S2 which will read each tag once only
        try {
            myReader.Actions.getReadTags(1);
            Antennas.SingulationControl s1_singulationControl = myReader.Config.Antennas.getSingulationControl(1);
            s1_singulationControl.setSession(SESSION.SESSION_S2);
            s1_singulationControl.Action.setInventoryState(INVENTORY_STATE.INVENTORY_STATE_A);
            s1_singulationControl.Action.setSLFlag(SL_FLAG.SL_ALL);
...
//                    ConfigureReader();
    ...
    public void ConfigureReaderExtern(RFIDReader myReader) {
        Log.d(TAG, "ConfigureReader " + myReader.getHostName());
        if (myReader.isConnected()) {
            ...
            try {
                ...
                if (externHandler == null)
                    externHandler = new EventHandler();
                myReader.Events.addEventsListener(externHandler);
                // HH event
                ...
                MAX_POWER = myReader.ReaderCapabilities.getTransmitPowerLevelValues().length - 1;
                // set antenna configurations
                Antennas.AntennaRfConfig config = myReader.Config.Antennas.getAntennaRfConfig(1);
                config.setTransmitPowerIndex(MAX_POWER);
                config.setrfModeTableIndex(0);
                config.setTari(0);
                myReader.Config.Antennas.setAntennaRfConfig(1, config);
